...
public class Main
{
    ...
    /**
     * myOpinion 处理启动参数和配置文件，
     *           参数来源三个地方：1、启动时cmd参数  2、start.ini 3.start.d目录下的配置文件
     *                            4、.ini 文件配置的--module指名的modules下的mod文件配置的信息
     * 				
     * @param cmdLine
     * @return
     * @throws Exception
    ...
    public StartArgs processCommandLine(String[] cmdLine) throws Exception
    {
        ...
        // ------------------------------------------------------------
        // 2) Start Logging 
        // myOpinion 设置jetty.home jetty.base
        ...
        // ------------------------------------------------------------
        // 3) Load Inis
        // myOpinion 读取jetty home目录下的配置文件start.ini 和 start.d目录下的所有后缀为ini的配置文件
        File start_ini = baseHome.getBaseFile("start.ini");
        ...
        // 5) Module Registration 注册所有在modules文件夹下的module，生成对象类
        Modules modules = new Modules();
        StartLog.debug("Registering all modules");
        modules.registerAll(baseHome);

        // 6) Active Module Resolution  将配置的module设置为enable，并设置来源的配置文件路径
