...
public final class SimpleWebApplicationServiceImpl extends
    ...
    public static SimpleWebApplicationServiceImpl createServiceFrom(
        ...
        if (!StringUtils.hasText(serviceToUse)) {
            String defUrl = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + "/portal";
            serviceToUse = SystemConfigProperties.get("login.service.defaultUrl", defUrl);
            String ip = request.getServerName();
            serviceToUse = serviceToUse.replace("IP", ip);
        }
